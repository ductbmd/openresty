...
local res, err =  red:hmget(cookie_value, 'id', 'logId', 'isSuperAdmin', 'ipAddress')
--Chưa có trong redis:
...
if tostring(res[1])=='userdata: NULL' then
    ...
    ngx.req.set_header("logId", res[2])
    ngx.req.set_header("isSuperAdmin", res[3])
    ngx.req.set_header("ip", res[4])

    ngx.var.target = ngx.var.uri
end
