...
local res, err =  red:hmget(cookie_value, 'username','id','token','partnerid')
--Chưa có trong redis:
...
if tostring(res[1])=='userdata: NULL' then
    ...
    ngx.req.set_header("token", "abc")
    ngx.req.set_header("partnerid", 1)
    if ngx.var.query_string==nil then
        ngx.var.target = '/app/info_sky_v2'..string.sub (ngx.var.uri,7)
    else
        ngx.var.target = '/app/info_sky_v2' .. string.sub (ngx.var.uri,7).. '?' .. ngx.var.query_string
    end
end
